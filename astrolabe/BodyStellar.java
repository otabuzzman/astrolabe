...
public class BodyStellar extends astrolabe.model.BodyStellar {
	...
	public BodyStellar( Object peer, Projector projector ) throws ParameterNotValidException {
		...
		size = ApplicationHelper.getClassNode( this,
				getGlyph().getName(), null ).getDouble( ApplicationConstant.PN_BODY_SIZE, DEFAULT_SIZE ) ;

		turn = -CAACoordinateTransformation.HoursToDegrees( getGlyph().getTurn() ) ;
		spin = -CAACoordinateTransformation.HoursToDegrees( getGlyph().getSpin() ) ;

		lo = AstrolabeFactory.valueOf( getPosition() ) ;
		az = lo[1] ;
		al = lo[2] ;

		eq = projector.convert( az, al ) ;
	...
	public void emitPS( PostscriptStream ps ) throws ParameterNotValidException {
		...
		text.setValue( getGlyph().getValue() ) ;
